...
static int __nex_driver_io(struct ifreq *ifr, struct nex_ioctl *ioc) {
  int s;
  int ret = 0;
...
static int __nex_driver_udp(struct nexio *nexio, struct nex_ioctl *ioc) {
  ...
  if (ioc->set == 0 && rx_frame_len > 0 && *(unsigned int *)frame == ioc->cmd) {
    memcpy(ioc->buf, ((char *)frame) + sizeof(frame->cmd) + sizeof(frame->set),
           (rx_frame_len - sizeof(frame->cmd) - sizeof(frame->set)) < ioc->len
               ? (rx_frame_len - sizeof(frame->cmd) - sizeof(frame->set))
               : ioc->len);
  }
...
static int __nex_driver_netlink(struct nexio *nexio, struct nex_ioctl *ioc) {
  int frame_len = ioc->len + sizeof(struct nexudp_ioctl_header) - sizeof(char);
  int rx_frame_len = 0;
  struct nexudp_ioctl_header *frame;
  int ret = 0;
  ...
  //	printf("%s: framelen %d %d %s\n", __FUNCTION__, rx_frame_len,
  // nlh->nlmsg_len, (char *) frame);

  if (ioc->set == 0 && rx_frame_len > 0 && frame->cmd == ioc->cmd) {
    memcpy(
        ioc->buf, frame->payload,
        (rx_frame_len - sizeof(struct nexudp_ioctl_header) + sizeof(char)) <
                ioc->len
            ? (rx_frame_len - sizeof(struct nexudp_ioctl_header) + sizeof(char))
            : ioc->len);
  }
 ...
 * ioctl_queryinformation_fe for executing  remote commands or local commands
 */
...
struct nexio *nex_init_udp(unsigned int securitycookie, unsigned int txip) {
  ...
  struct sockaddr_in *sin_rx_ioctl =
      (struct sockaddr_in *)malloc(sizeof(struct sockaddr_in));
  struct sockaddr_in *sin_rx_frame =
      (struct sockaddr_in *)malloc(sizeof(struct sockaddr_in));
  ...
  nexio->sock_rx_ioctl =
      socket(sin_rx_ioctl->sin_family, SOCK_DGRAM, IPPROTO_UDP);
  nexio->sock_rx_frame =
      socket(sin_rx_frame->sin_family, SOCK_DGRAM, IPPROTO_UDP);

  // Enable broadcasts for transmit socket
  int broadcast_enable = 1;
  setsockopt(nexio->sock_tx, SOL_SOCKET, SO_BROADCAST, &broadcast_enable,
  ...
  bind(nexio->sock_rx_ioctl, (struct sockaddr *)sin_rx_ioctl,
       sizeof(struct sockaddr));
  bind(nexio->sock_rx_frame, (struct sockaddr *)sin_rx_frame,
       sizeof(struct sockaddr));
...
struct nexio *nex_init_netlink(void) {
  ...
  struct sockaddr_nl *snl_tx =
      (struct sockaddr_nl *)malloc(sizeof(struct sockaddr_nl));
  struct sockaddr_nl *snl_rx_ioctl =
      (struct sockaddr_nl *)malloc(sizeof(struct sockaddr_nl));
  ...
  err = bind(nexio->sock_rx_ioctl, (struct sockaddr *)snl_rx_ioctl,
             sizeof(struct sockaddr));
  if (err)
    fprintf(stderr, "%s: bind error (%d: %s)\n", __FUNCTION__, errno,
            strerror(errno));

  err = connect(nexio->sock_tx, (struct sockaddr *)snl_tx,
                sizeof(struct sockaddr));
  if (err)
